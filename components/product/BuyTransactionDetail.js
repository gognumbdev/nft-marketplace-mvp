...
const TransactionDetail = ({nftData,maticPerUSD}) => {
  ...
  return (
    <div className="w-full p-4 grid grid-cols-1" >
        ...
        {/*User Balance */}
        <div className="flex-col justify-start font-medium mt-2">
            <div className="flex space-x-10  items-center">
                <p className="">Your Balance</p>
                    
        
                {/* Cryptocurrency Price */}
                <p className="">{userBalance} {unit}</p>
            </div>

            {/* USD price */}
            <p className="ml-[139px] text-gray-500"> ≈ ${(userBalance*maticPerUSD).toFixed(2)}</p>
        </div>
