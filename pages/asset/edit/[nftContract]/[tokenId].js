...
const EditItemInfo = ({nftData}) => {
  ...
  const {nftName,description,price,blockchain,blockchainImage,unit,tokenId,ownerWalletAddress} = nftData
  ...
  const submitEditItemInfo = async () => {
    ...
    let res = await fetch(`http://localhost:3000/api/asset/edit/${nftContract}/${tokenId}`,{
        method:"POST",
    ...
    router.push(`/profile/${ownerWalletAddress}`)
...
export async function getServerSideProps(context) {
  let {nftContract,tokenId} = context.query;
  const res = await fetch(`http://localhost:3000/api/asset/edit/${nftContract}/${tokenId}`)
